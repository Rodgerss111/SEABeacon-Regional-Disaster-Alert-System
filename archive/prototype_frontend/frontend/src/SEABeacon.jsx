...
const PROVINCES = {
  ...
  VN: ["Hanoi","Ho Chi Minh City","Da Nang","Hai Phong","Can Tho","An Giang","Ben Tre","Ca Mau","Khanh Hoa","Nghe An","Quang Binh","Quang Nam","Thua Thien Hue"],
...
// ══ NAME NORMALIZATION ════════════════════════════════════════════════════════
// The three AI backends don't agree on spellings: XGBoost emits country
// "Philippines" while the UI/NLP use the ISO code "VN", and province names
// arrive with/without spaces and diacritics ("QuảngBình" vs "Quang Binh").
// Fusion groups by `${country}::${province}`, so un-normalized inputs silently
// split one province into several rankings. Every report flows through
// normalizeReport() at ingest (in handleSubmit) before fusion.

const COUNTRY_ALIASES = {
  ph:"PH", phl:"PH", philippines:"PH",
  vn:"VN", vnm:"VN", vietnam:"VN", "viet nam":"VN", vt:"VN", // VT = legacy alias
...
// Loose key: strip diacritics (incl. Vietnamese đ), drop non-alphanumerics.
// "Quảng Bình" / "QuảngBình" / "quang binh" all collapse to "quangbinh".
function looseKey(s) {
  return String(s)
...
// looseKey -> canonical spelling, built from the UI's province lists.
const CANONICAL_PROVINCE = (() => {
  const map = {};
  for (const list of Object.values(PROVINCES)) {
    for (const name of list) map[looseKey(name)] = name;
  }
  return map;
})();
...
function normalizeProvince(province) {
  ...
  const cleaned = String(province).trim().replace(/\s+/g, " ");
  return CANONICAL_PROVINCE[looseKey(cleaned)] || cleaned; // fall back to cleaned original
}

// Returns the report with canonical country + province (other fields untouched).
function normalizeReport(report) {
  return {
    ...report,
    country: normalizeCountry(report.country),
    province: normalizeProvince(report.province),
  };
}
...
const SIM_SCRIPT = [
  ...
  },
  {
    ...
    flood:   { country:"VN", provinces:["Quang Binh","Quang Nam","Thua Thien Hue"],               scoreRange:[0.65,0.78], ctx:{ basinName:"Thu Bon Basin",   discharge:"1200", threshold:"1600", floodHorizon:"36", severityLabel:"ADVISORY" }},
    typhoon: { country:"VN", provinces:["Quang Binh","Quang Nam"],                                scoreRange:[0.60,0.74], ctx:{ stormName:"Bão số 4",        windKmh:"130", typhoonHorizon:"36", cteKm:"130" }},
    social:  { country:"VN", provinces:["Quang Binh","Quang Nam","Thua Thien Hue"],               scoreRange:[0.58,0.72], ctx:{ stormName:"Bão số 4",        postCount:"29",  domLabel:"advisory", highLang:true }},
  },
  {
    ...
    <div style={{ background:C.surface, border:`0.5px solid ${C.border}`, borderRadius:14,
      ...
      <div style={{ marginBottom:10 }}>
        ...
        <select value={country} onChange={e=>setCountry(e.target.value)}
          ...
          {["PH","VN","TH"].map(c=><option key={c}>{c}</option>)}
        </select>
      </div>
...
export default function SEABeacon({ selectedProvince, onRankedUpdate, hideImpactMap = false }) {
  ...
  const handleSubmit = useCallback(async (report) => {
    // Canonicalize country + province so all sources (manual + the three AI
    // pollers) agree on spelling before fusion groups them by `country::province`.
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // AI-1 (Flood / LSTM) processor: polls flood_predictions and converts each
  // basin prediction into a "flood" report. Mirrors the AI-2 poller above.
  // (demo2 integration — wires the LSTM backend into the live fusion engine.)
  // ──────────────────────────────────────────────────────────────────────────
  useEffect(() => {
    if (!AI1_SUPABASE_URL || !AI1_SUPABASE_ANON_KEY) return; // skip if unconfigured
    let isMounted = true;
    const seenFlood = new Set(); // dedup: `${province}::${run_timestamp}::${basin_id}`

    const fetchFloodPredictions = async () => {
      try {
        const resp = await fetch(
          `${AI1_SUPABASE_URL}/rest/v1/flood_predictions?select=*&order=run_timestamp.desc&limit=60`,
          { headers: { apikey: AI1_SUPABASE_ANON_KEY, Authorization: `Bearer ${AI1_SUPABASE_ANON_KEY}` } }
        );
        if (!resp.ok) {
          console.error("Flood poll failed:", resp.status, resp.statusText);
          return;
        }
        ...
        rows.reverse().forEach((row) => {
          if (!isMounted) return;
          const key = `${row.province}::${row.run_timestamp ?? ""}::${row.basin_id ?? ""}`;
          if (seenFlood.has(key)) return;
          seenFlood.add(key);

          const score = typeof row.score_value === "number"
            ? Math.max(0.01, Math.min(0.99, row.score_value))
            : null;
          if (score === null || !row.province || !row.country) return;
          ...
          handleSubmit({
            ...
            highLang: true,
            ctx: {
              basinName: (row.basin_id || "").replace(/_/g, " "),
              discharge: row.discharge_cms,
              threshold: row.threshold_cms,
              floodHorizon: row.forecast_hrs,
              severityLabel: row.severity_label,
              stormName: row.typhoon_name || undefined,
            },
          });
        });
      } catch (err) {
        console.error("Error in flood processor:", err);
      }
    };

    const id = setInterval(fetchFloodPredictions, 30000);
    fetchFloodPredictions();
    return () => { isMounted = false; clearInterval(id); };
  }, [handleSubmit, nextId, tsDate, AI1_SUPABASE_URL, AI1_SUPABASE_ANON_KEY]);

  // ──────────────────────────────────────────────────────────────────────────
  // AI-3 (Social / NLP) processor: polls the `alerts` table and fans each
  // classified article out into one "social" report per affected province.
  // (demo2 integration — wires the NLP backend into the live fusion engine.)
  // ──────────────────────────────────────────────────────────────────────────
  useEffect(() => {
    if (!AI3_SUPABASE_URL || !AI3_SUPABASE_ANON_KEY) return; // skip if unconfigured
    let isMounted = true;
    const seenSocial = new Set(); // dedup: `${article_url}::${province}`

    const fetchSocialAlerts = async () => {
      try {
        const resp = await fetch(
          `${AI3_SUPABASE_URL}/rest/v1/alerts?select=*&order=created_at.desc&limit=80`,
          { headers: { apikey: AI3_SUPABASE_ANON_KEY, Authorization: `Bearer ${AI3_SUPABASE_ANON_KEY}` } }
        );
        if (!resp.ok) {
          console.error("Social poll failed:", resp.status, resp.statusText);
          return;
        }
        const rows = await resp.json();
        if (!Array.isArray(rows) || rows.length === 0) return;
        ...
        rows.reverse().forEach((row) => {
          ...
          const score = typeof row.score === "number"
            ? Math.max(0.01, Math.min(0.99, row.score))
            : null;
          if (score === null || !row.country) return;

          // `provinces` is a JSON array on the alert row; fan out one report each.
          const provinces = Array.isArray(row.provinces) ? row.provinces : [];
          provinces.forEach((prov) => {
            if (!prov) return;
            const key = `${row.article_url ?? row.id ?? ""}::${prov}`;
            ...
            handleSubmit({
              ...
              submittedAt: Date.now(),
              displayTime: tsDate(),
              highLang: true, // NLP emits no language-resource flag; default to 35% weight
              ctx: {
                stormName: row.storm_name || undefined,
                postCount: row.source || "news",
                domLabel: row.alert_level || row.hazard,
              },
            });
          });
        });
      } catch (err) {
        console.error("Error in social processor:", err);
      }
    };

    const id = setInterval(fetchSocialAlerts, 30000);
    fetchSocialAlerts();
    return () => { isMounted = false; clearInterval(id); };
